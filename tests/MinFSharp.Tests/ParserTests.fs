...
//                d "=" (Var <| Id "=")
                ...
                d "let x = 7 in\nx" (LetIn(((Id "x"), Type.Var None), Int 7, Var(Id "x")))
                d "true" (Bool true)
                d "false" (Bool false)
                ...
                d "let min x y = if x < y then x else y" (Let((Id "min", Type.Var None), (FunDef([(Id "x", Type.Var None);(Id "y", Type.Var None)], FBody.Body ((If (BinOp ("<",Var (Id "x"),Var (Id "y")),Var (Id "x"),Var (Id "y"))))))))
                d "let f x y = y" (Let((Id "f", Type.Var None), (FunDef([(Id "x", Type.Var None);(Id "y", Type.Var None)], FBody.Body << Var <| Id "y"))))
            |]
